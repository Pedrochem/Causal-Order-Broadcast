...
func (module *PP2PLink) Start(address string) {
	...
	go func() {
		for {
			// recebe o Request e ativa a funcao Wait
			message := <-module.Req
			go module.Wait(message)
		}
	}()
}

// caso a mensagem contenha a palavra 'delay' seguido da porta do processo destino (ex delay 7001)
// a mensagem ira sofrer um atraso de 15 segundos
func (module *PP2PLink) Wait(message PP2PLink_Req_Message) {
